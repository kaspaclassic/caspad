...
func sign(conf *signConfig) error {
	...
	for i, partiallySignedTransaction := range partiallySignedTransactions {
		updatedPartiallySignedTransactions[i], err =
			libcaswallet.Sign(conf.NetParams(), privateKeys, partiallySignedTransaction, keysFile.ECDSA)
		if err != nil {
			return err
		}
	}
	...
	for _, updatedPartiallySignedTransaction := range updatedPartiallySignedTransactions {
		...
		isFullySigned, err := libcaswallet.IsTransactionFullySigned(updatedPartiallySignedTransaction)
		if err != nil {
			return err
		}
		if !isFullySigned {
