...
func (s *server) mergeTransaction(
	...
	utxos := make([]*libcaswallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &libcaswallet.UTXO{
	...
	payments := []*libcaswallet.Payment{{
		Address: toAddress,
		Amount:  sentValue,
	}}
	if totalValue > sentValue {
		payments = append(payments, &libcaswallet.Payment{
			Address: changeAddress,
			Amount:  totalValue - sentValue,
		})
	}

	mergeTransactionBytes, err := libcaswallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by caswallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*libcaswallet.UTXO, 0, endIndex-startIndex)
	totalLeor := uint64(0)
	...
	for i := startIndex; i < endIndex && i < len(transaction.PartiallySignedInputs); i++ {
		...
		selectedUTXOs = append(selectedUTXOs, &libcaswallet.UTXO{
	...
	unsignedTransactionBytes, err := libcaswallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) estimateMassAfterSignatures(transaction *serialization.PartiallySignedTransaction) (uint64, error) {
	...
	transactionWithSignatures, err := libcaswallet.ExtractTransactionDeserialized(transaction, s.keysFile.ECDSA)
	if err != nil {
		return 0, err
	}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libcaswallet.UTXO, requiredAmount uint64) (
	additionalUTXOs []*libcaswallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &libcaswallet.UTXO{
