...
func (s *server) changeAddress(useExisting bool, fromAddresses []*walletAddress) (util.Address, *walletAddress, error) {
	...
	if len(fromAddresses) != 0 && useExisting {
		...
		walletAddr = &walletAddress{
			...
			keyChain:      libcaswallet.InternalKeychain,
		}
	}

	path := s.walletAddressPath(walletAddr)
	address, err := libcaswallet.Address(s.params, s.keysFile.ExtendedPublicKeys, s.keysFile.MinimumSignatures, path, s.keysFile.ECDSA)
	if err != nil {
		return nil, nil, err
	}
...
//my-add		return nil, errors.Errorf("wallet daemon is not synced yet, %s", s.formatSyncStateReport())
	...
	for i := uint32(1); i <= s.keysFile.LastUsedExternalIndex(); i++ {
		walletAddr := &walletAddress{
			index:         i,
			cosignerIndex: s.keysFile.CosignerIndex,
			keyChain:      libcaswallet.ExternalKeychain,
		}
		path := s.walletAddressPath(walletAddr)
		address, err := libcaswallet.Address(s.params, s.keysFile.ExtendedPublicKeys, s.keysFile.MinimumSignatures, path, s.keysFile.ECDSA)
		if err != nil {
...
func (s *server) walletAddressString(wAddr *walletAddress) (string, error) {
	...
	addr, err := libcaswallet.Address(s.params, s.keysFile.ExtendedPublicKeys, s.keysFile.MinimumSignatures, path, s.keysFile.ECDSA)
	if err != nil {
		return "", err
	}
