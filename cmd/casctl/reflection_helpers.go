...
// generatecaspadMessage generates a wrapped caspadMessage with the given `commandValue`
func generatecaspadMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.PyipadMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	caspadMessage := reflect.New(reflect.TypeOf(protowire.PyipadMessage{}))
	caspadMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return caspadMessage.Interface().(*protowire.PyipadMessage), nil
}
