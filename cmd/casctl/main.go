...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		caspadMessage, err := client.Post(&protowire.PyipadMessage{Payload: &protowire.PyipadMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := caspadMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	caspadMessage := &protowire.PyipadMessage{}
	err := protojson.Unmarshal([]byte(response), caspadMessage)
	if err != nil {
	...
	return marshalOptions.Format(caspadMessage)
}
