...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-caspad.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "caspad")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of caspad binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-caspad.conf")
