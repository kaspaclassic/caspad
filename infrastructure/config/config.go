...
const (
	defaultConfigFilename      = "cas.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "cas.log"
	defaultErrLogFilename      = "cas_err.log"
	...
	sampleConfigFilename    = "sample-cas.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for caspad.
	DefaultAppDir = util.AppDir("cas", false)
...
// Flags defines the configuration options for caspad.
//
...
type Flags struct {
	...
	Listeners                       []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 17111, testnet: 16211)"`
	...
	RPCListeners                    []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 17110, testnet: 16210)"`
...
// Config defines the configuration options for caspad.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default caspad configuration
...
//
// The above results in caspad functioning properly without any config settings
...
// createDefaultConfig copies the file sample-caspad.conf to the given destination path,
