...
// Outpoint defines a caspa data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new caspa transaction outpoint point with the
...
// TxIn defines a caspa transaction input.
type TxIn struct {
...
// NewTxIn returns a new caspa transaction input with the provided
...
// TxOut defines a caspa transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new caspa transaction output with the provided
...
// MsgTx implements the Message interface and represents a caspa tx message.
