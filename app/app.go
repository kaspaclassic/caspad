...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "caspadsvc",
	DisplayName: "caspad Service",
	Description: "Downloads and stays synchronized with the Pyrin blockDAG and " +
		"provides DAG services to applications.",
}

type caspadApp struct {
	cfg *config.Config
}

// StartApp starts the caspad app, and blocks until it finishes running
func StartApp() error {
...
func (app *caspadApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start caspad: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down caspad...")
		...
		log.Infof("caspad shutdown complete")
	}()
