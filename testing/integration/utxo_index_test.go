...
func TestUTXOIndex(t *testing.T) {
	// Setup a single caspad instance
	...
	err := caspad.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := caspad.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedLeor := uint64(blockAmountToMine * constants.LeorPerPyrin * 17)
	getBlockCountResponse, err := caspad.rpcClient.GetBlockCount()
	if err != nil {
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = caspad.rpcClient.SubmitTransaction(rpcTransaction, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := caspad.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
