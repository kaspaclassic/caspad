...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("caspad-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("caspad-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("caspad-SYNCER",
		"caspad",
	...
	syncedCmd, err := common.StartCmd("caspad-SYNCED",
		"caspad",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
