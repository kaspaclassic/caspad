...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("caspad-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("caspad datadir: %s", dataDir)

	caspadCmd, err := common.StartCmd("caspad",
		"caspad",
	...
	spawn("startNode-caspadCmd.Wait", func() {
		err := caspadCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("caspadCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes caspad closes ungracefully
				log.Errorf("caspadCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(caspadCmd, "caspadCmd")
