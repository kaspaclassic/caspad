...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("caspadsanity-caspad-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("caspad", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "caspad-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "caspad-STDERR")
